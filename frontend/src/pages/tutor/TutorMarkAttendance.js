...
import { getMySubjects, getMyClass, getStudentsByClass, markAttendance, getMyTimetable } from '../../services/api';
...
const TutorMarkAttendance = () => {
  ...
  useEffect(() => {
    ...
    getMyClass().then(({ data }) => {
      if (data.classId)
        getStudentsByClass(data.classId).then(({ data: st }) => {
          setStudents(st);
          const init = {};
          st.forEach(s => (init[s.registerNumber] = true));
          setAttendance(init);
        }).catch(() => {});
    }).catch(() => {});
  }, []);
  ...
  const markAll = (val) => {
    const updated = {};
    students.forEach(st => (updated[st.registerNumber] = val));
    setAttendance(updated);
  };

  const handleSubmit = async () => {
    if (!selectedSubject) { setError('Please select a subject'); return; }
    if (validHours.length === 0) { setError('No classes scheduled for this subject today'); return; }
    if (!selectedHour) { setError('Please select a class hour'); return; }
    setSaving(true); setError(''); setSaved(false);
    try {
      await markAttendance({
        records: students.map(st => ({ studentId: st.registerNumber, isPresent: attendance[st.registerNumber] })),
        subjectId: parseInt(selectedSubject),
        date: today,
        classHour: parseInt(selectedHour),
      });
      setSaved(true);
            ...
            <div style={s.list}>
              {students.map((st, i) => (
                <div key={st.registerNumber} style={{ ...s.row,
                  background: attendance[st.registerNumber] ? 'rgba(76,175,80,0.1)' : 'rgba(244,67,54,0.08)',
                  border: `1px solid ${attendance[st.registerNumber] ? 'rgba(76,175,80,0.3)' : 'rgba(244,67,54,0.25)'}`,
                }}>
                  ...
                  <button style={{ ...s.toggle,
                    background: attendance[st.registerNumber] ? 'rgba(76,175,80,0.25)' : 'rgba(244,67,54,0.2)',
                    color: attendance[st.registerNumber] ? '#69f0ae' : '#ff8a80',
                    border: `1px solid ${attendance[st.registerNumber] ? '#4caf50' : '#f44336'}`,
                  }} onClick={() => toggle(st.registerNumber)}>
                    {attendance[st.registerNumber] ? '✓ Present' : '✗ Absent'}
                  </button>
                </div>
              ))}
            </div>
...
const s = {
  ...
  toggle: { padding: '7px 18px', borderRadius: '20px', cursor: 'pointer', fontWeight: 600, fontSize: '13px' },
};
